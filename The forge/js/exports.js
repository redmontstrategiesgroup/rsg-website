...
// user/AI-influenced strings must be escaped before HTML interpolation
...
export function manualHTML(spec) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${esc(spec.name)} — Assembly Manual</title>
<style>
...
<div class="hero"><div>
<h1>${esc(spec.name.toUpperCase())}</h1>
<p class="tag">${esc(spec.tagline)}</p>
<a class="cta" href="#build">Build it yourself — full plans included</a>
</div></div>
${renderDataURL ? `<div class="render"><img src="${renderDataURL}" alt="${esc(spec.name)} render"></div>` : ""}
<div class="grid" id="build">
 ...
 <div class="card"><h3>👤 Fits YOUR hand</h3><p>Generated from a physical-ability profile: ${esc(spec.ability.hand)} hand, reach ${spec.ability.reachMM} mm. Every deck is personal.</p></div>
