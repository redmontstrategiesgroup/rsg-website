...
const POST = withApi("POST", { auth: "client", scopes: ["tickets:write"], idempotent: true, body, meta: { operationId: "createX", summary: "Create", tag: "Tickets", response: z.object({ data: z.object({ id: z.string() }) }), status: 201, extraResponses: { "200": "Duplicate" } } }, async () => ({ data: {} }), deps);
...
describe("openApiForRoutes", () => {
  ...
  it("emits operations with security, scopes, params, body example and error refs", () => {
    ...
    assert.equal(post.responses["200"].description, "Duplicate");
    assert.deepEqual(post.responses["200"].content["application/json"].schema, post.responses["201"].content["application/json"].schema);
    assert.ok(get.responses["200"], "default status is 200");
